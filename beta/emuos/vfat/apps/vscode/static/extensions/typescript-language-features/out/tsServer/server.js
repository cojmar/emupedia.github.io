...
class SyntaxRoutingTsServer extends dispose_1.Disposable {
    constructor(syntaxServer, semanticServer, _delegate) {
        super();
    ...
    executeImpl(command, args, executeInfo) {
        if (SyntaxRoutingTsServer.syntaxCommands.has(command)) {
            ...
            let syntaxRequestState = 0 /* Unresolved */;
            let semanticRequestState = 0 /* Unresolved */;
            // Also make sure we never cancel requests to just one server
            let token = undefined;
            if (executeInfo.token) {
                const source = new vscode.CancellationTokenSource();
                executeInfo.token.onCancellationRequested(() => {
                    if (syntaxRequestState !== 0 /* Unresolved */ && semanticRequestState === 0 /* Unresolved */
                        || syntaxRequestState === 0 /* Unresolved */ && semanticRequestState !== 0 /* Unresolved */) {
                        // Don't cancel.
            ...
            if (semanticRequest) {
                semanticRequest
                    .then(result => {
                    semanticRequestState = 1 /* Resolved */;
                    if (syntaxRequestState === 2 /* Errored */) {
                        // We've gone out of sync
                        this._delegate.onFatalError(command);
                    }
                    return result;
                }, err => {
                    semanticRequestState = 2 /* Errored */;
                    if (syntaxRequestState === 1 /* Resolved */) {
                        // We've gone out of sync
                        this._delegate.onFatalError(command);
                    }
                    throw err;
            ...
            if (syntaxRequest) {
                syntaxRequest
                    .then(result => {
                    syntaxRequestState = 1 /* Resolved */;
                    if (semanticRequestState === 2 /* Errored */) {
                        // We've gone out of sync
                        this._delegate.onFatalError(command);
                    }
                    return result;
                }, err => {
                    syntaxRequestState = 2 /* Errored */;
                    if (semanticRequestState === 1 /* Resolved */) {
                        // We've gone out of sync
                        this._delegate.onFatalError(command);
                    }
                    throw err;
