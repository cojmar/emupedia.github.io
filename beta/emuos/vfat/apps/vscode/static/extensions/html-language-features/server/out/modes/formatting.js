...
function format(languageModes, document, formatRange, formattingOptions, settings, enabledModes) {
    ...
    if (endPos.character === 0 && endPos.line > 0 && endOffset !== content.length) {
        ...
        let prevLineStart = document.offsetAt(vscode_html_languageservice_1.Position.create(endPos.line - 1, 0));
        while (strings_1.isEOL(content, endOffset - 1) && endOffset > prevLineStart) {
            endOffset--;
        }
        formatRange = vscode_html_languageservice_1.Range.create(formatRange.start, document.positionAt(endOffset));
    }
    ...
    while (i < allRanges.length && !isHTML(allRanges[i])) {
        ...
        if (!range.attributeValue && range.mode && range.mode.format) {
            let edits = range.mode.format(document, vscode_html_languageservice_1.Range.create(startPos, range.end), formattingOptions, settings);
    ...
    // modify the range
    formatRange = vscode_html_languageservice_1.Range.create(startPos, formatRange.end);
    // perform a html format and apply changes to a new document
    let htmlMode = languageModes.getMode('html');
    let htmlEdits = htmlMode.format(document, formatRange, formattingOptions, settings);
    let htmlFormattedContent = vscode_html_languageservice_1.TextDocument.applyEdits(document, htmlEdits);
    let newDocument = vscode_html_languageservice_1.TextDocument.create(document.uri + '.tmp', document.languageId, document.version, htmlFormattedContent);
    try {
        ...
        let newFormatRange = vscode_html_languageservice_1.Range.create(formatRange.start, newDocument.positionAt(htmlFormattedContent.length - afterFormatRangeLength));
        ...
        let resultContent = vscode_html_languageservice_1.TextDocument.applyEdits(newDocument, embeddedEdits);
        ...
        result.push(vscode_html_languageservice_1.TextEdit.replace(formatRange, resultReplaceText));
        return result;
    }
    finally {
