...
suite('HTML Embedded Formatting', () => {
    function assertFormat(value, expected, options, formatOptions, message) {
        ...
        let document = vscode_html_languageservice_1.TextDocument.create('test://test/test.html', 'html', 0, value);
        let range = vscode_html_languageservice_1.Range.create(document.positionAt(rangeStartOffset), document.positionAt(rangeEndOffset));
        if (!formatOptions) {
            formatOptions = vscode_html_languageservice_1.FormattingOptions.create(2, true);
        }
        let result = formatting_1.format(languageModes, document, range, formatOptions, undefined, { css: true, javascript: true });
        let actual = vscode_html_languageservice_1.TextDocument.applyEdits(document, result);
    ...
    test('HTLM & Scripts - Fixtures', function () {
        ...
        assertFormatWithFixture('19813.html', '19813-4spaces.html', undefined, vscode_html_languageservice_1.FormattingOptions.create(4, true));
        assertFormatWithFixture('19813.html', '19813-tab.html', undefined, vscode_html_languageservice_1.FormattingOptions.create(1, false));
