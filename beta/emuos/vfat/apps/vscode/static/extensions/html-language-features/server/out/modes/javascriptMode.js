    ...
    return {
        ...
        doValidation(document) {
            ...
            return syntaxDiagnostics.concat(semanticDiagnostics).map((diag) => {
                return {
                    ...
                    severity: vscode_html_languageservice_1.DiagnosticSeverity.Error,
                    source: 'js',
            ...
            return {
                ...
                items: completions.entries.map(entry => {
                    return {
                        ...
                        textEdit: vscode_html_languageservice_1.TextEdit.replace(replaceRange, entry.name),
                        data: {
        ...
        doHover(document, position) {
            ...
            if (info) {
                ...
                return {
                    ...
                    contents: vscode_html_languageservice_1.MarkedString.fromPlainText(contents)
                };
            }
            return null;
        },
            ...
            for (const entry of highlights || []) {
                for (const highlight of entry.highlightSpans) {
                    out.push({
                        ...
                        kind: highlight.kind === 'writtenReference' ? vscode_html_languageservice_1.DocumentHighlightKind.Write : vscode_html_languageservice_1.DocumentHighlightKind.Text
                    });
                }
            }
            return out;
        ...
        format(document, range, formatParams, settings = globalSettings) {
            ...
            if (range.end.line > range.start.line && (range.end.character === 0 || strings_1.isWhitespaceOnly(currentTextDocument.getText().substr(end - range.end.character, range.end.character)))) {
                ...
                lastLineRange = vscode_html_languageservice_1.Range.create(vscode_html_languageservice_1.Position.create(range.end.line, 0), range.end);
            }
        ...
        getFoldingRanges(document) {
            ...
            for (let span of spans) {
                ...
                if (startLine < endLine) {
                    ...
                    if (match) {
                        foldingRange.kind = match[1] ? vscode_html_languageservice_1.FoldingRangeKind.Region : vscode_html_languageservice_1.FoldingRangeKind.Comment;
                    }
...
function convertRange(document, span) {
    if (typeof span.start === 'undefined') {
        ...
        return vscode_html_languageservice_1.Range.create(pos, pos);
    }
    const startPosition = document.positionAt(span.start);
    const endPosition = document.positionAt(span.start + (span.length || 0));
    return vscode_html_languageservice_1.Range.create(startPosition, endPosition);
}
function convertKind(kind) {
    switch (kind) {
        case 'primitive type':
        case 'keyword':
            return vscode_html_languageservice_1.CompletionItemKind.Keyword;
        case 'var':
        case 'local var':
            return vscode_html_languageservice_1.CompletionItemKind.Variable;
        case 'property':
        case 'getter':
        case 'setter':
            return vscode_html_languageservice_1.CompletionItemKind.Field;
        case 'function':
        case 'method':
        case 'construct':
        case 'call':
        case 'index':
            return vscode_html_languageservice_1.CompletionItemKind.Function;
        case 'enum':
            return vscode_html_languageservice_1.CompletionItemKind.Enum;
        case 'module':
            return vscode_html_languageservice_1.CompletionItemKind.Module;
        case 'class':
            return vscode_html_languageservice_1.CompletionItemKind.Class;
        case 'interface':
            return vscode_html_languageservice_1.CompletionItemKind.Interface;
        case 'warning':
            return vscode_html_languageservice_1.CompletionItemKind.File;
    }
    return vscode_html_languageservice_1.CompletionItemKind.Property;
}
function convertSymbolKind(kind) {
    switch (kind) {
        ...
        case 'local var':
        case 'const':
            return vscode_html_languageservice_1.SymbolKind.Variable;
        case 'function':
        case 'local function':
            return vscode_html_languageservice_1.SymbolKind.Function;
        case 'enum':
            return vscode_html_languageservice_1.SymbolKind.Enum;
        case 'module':
            return vscode_html_languageservice_1.SymbolKind.Module;
        case 'class':
            return vscode_html_languageservice_1.SymbolKind.Class;
        case 'interface':
            return vscode_html_languageservice_1.SymbolKind.Interface;
        case 'method':
            return vscode_html_languageservice_1.SymbolKind.Method;
        case 'property':
        case 'getter':
        case 'setter':
            return vscode_html_languageservice_1.SymbolKind.Property;
    }
    return vscode_html_languageservice_1.SymbolKind.Variable;
}
...
function computeInitialIndent(document, range, options) {
    let lineStart = document.offsetAt(vscode_html_languageservice_1.Position.create(range.start.line, 0));
