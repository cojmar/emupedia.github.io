...
function assertCompletion(completions, expected, document) {
    ...
    if (expected.resultText && match.textEdit) {
        assert.equal(vscode_html_languageservice_1.TextDocument.applyEdits(document, [match.textEdit]), expected.resultText);
    }
    if (expected.command) {
...
function testCompletionFor(value, expected, uri = testUri, workspaceFolders) {
    ...
    let document = vscode_html_languageservice_1.TextDocument.create(uri, 'html', 0, value);
...
suite('HTML Path Completion', () => {
    ...
    test('Basics - Correct label/kind/result/command', () => {
        testCompletionFor('<script src="./|">', {
            items: [
                { label: 'about/', kind: vscode_html_languageservice_1.CompletionItemKind.Folder, resultText: '<script src="./about/">', command: triggerSuggestCommand },
                { label: 'index.html', kind: vscode_html_languageservice_1.CompletionItemKind.File, resultText: '<script src="./index.html">' },
                { label: 'src/', kind: vscode_html_languageservice_1.CompletionItemKind.Folder, resultText: '<script src="./src/">', command: triggerSuggestCommand }
            ]
        }, indexHtmlUri);
    });
