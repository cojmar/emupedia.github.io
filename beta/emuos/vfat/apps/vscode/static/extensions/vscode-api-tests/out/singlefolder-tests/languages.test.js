...
suite('languages namespace tests', () => {
    ...
    test('setTextDocumentLanguage -> close/open event', async function () {
        const file = await utils_1.createRandomFile('foo\nbar\nbar');
    ...
    test('link detector', async function () {
        const uri = await utils_1.createRandomFile('class A { // http://a.com }', undefined, '.java');
    ...
    test('completions with document filters', async function () {
        let ran = false;
        let uri = vscode.Uri.file(path_1.join(vscode.workspace.rootPath || '', './bower.json'));
