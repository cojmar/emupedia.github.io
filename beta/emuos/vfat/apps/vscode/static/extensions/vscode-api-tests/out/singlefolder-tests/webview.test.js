...
const testDocument = path_1.join(vscode.workspace.rootPath || '', './bower.json');
suite('Webview tests', () => {
    const disposables = [];
    ...
    utils_1.conditionalTest('webviews should preserve vscode API state when they are hidden', async () => {
        const webview = _register(vscode.window.createWebviewPanel(webviewId, 'title', { viewColumn: vscode.ViewColumn.One }, { enableScripts: true }));
    ...
    utils_1.conditionalTest('webviews should preserve their context when they are moved between view columns', async () => {
        const doc = await vscode.workspace.openTextDocument(testDocument);
    ...
    utils_1.conditionalTest('webviews with retainContextWhenHidden should preserve their context when they are hidden', async () => {
    ...
    utils_1.conditionalTest('webviews with retainContextWhenHidden should preserve their page position when hidden', async () => {
        const webview = _register(vscode.window.createWebviewPanel(webviewId, 'title', { viewColumn: vscode.ViewColumn.One }, { enableScripts: true, retainContextWhenHidden: true }));
        const ready = getMesssage(webview);
        webview.webview.html = createHtmlDocumentWithBody(/*html*/ `
    ...
    utils_1.conditionalTest('webviews with retainContextWhenHidden should be able to recive messages while hidden', async () => {
        const webview = _register(vscode.window.createWebviewPanel(webviewId, 'title', { viewColumn: vscode.ViewColumn.One }, { enableScripts: true, retainContextWhenHidden: true }));
        const ready = getMesssage(webview);
        webview.webview.html = statefulWebviewHtml;
        await ready;
    ...
    utils_1.conditionalTest('webviews should only be able to load resources from workspace by default', async () => {
        const webview = _register(vscode.window.createWebviewPanel(webviewId, 'title', { viewColumn: vscode.ViewColumn.One }, { enableScripts: true }));
        webview.webview.html = createHtmlDocumentWithBody(/*html*/ `
			<script>
				const vscode = acquireVsCodeApi();
        ...
        async function asWebviewUri(path) {
        ...
        {
            const imagePath = vscode.Uri.file(path_1.join(vscode.workspace.rootPath, '..', '..', '..', 'resources', 'linux', 'code.png')).with({ scheme: 'vscode-resource' });
            const response = sendRecieveMessage(webview, { src: imagePath.toString() });
            assert.strictEqual((await response).value, false);
        }
    });
    utils_1.conditionalTest('webviews should allow overriding allowed resource paths using localResourceRoots', async () => {
        const webview = _register(vscode.window.createWebviewPanel(webviewId, 'title', { viewColumn: vscode.ViewColumn.One }, {
            enableScripts: true,
            localResourceRoots: [vscode.Uri.file(path_1.join(vscode.workspace.rootPath, 'sub'))]
        }));
        webview.webview.html = createHtmlDocumentWithBody(/*html*/ `
			<script>
				const vscode = acquireVsCodeApi();
    ...
    test('webviews should have real view column after they are created, #56097', async () => {
        ...
        let changed = false;
        const viewStateChanged = new Promise((resolve) => {
...
function createHtmlDocumentWithBody(body) {
...
function sendRecieveMessage(webview, message) {
    const p = getMesssage(webview);
